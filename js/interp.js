...
function branchRelations(pillars){
    ...
    for (let j=i+1;j<keys.length;j++){
      ...
      if (((a-b)%12+12)%12===6){
        out.push({type:"충", a,b,posA:ka,posB:kb, priority:9,
          easy:`${tag} 충(沖) — 이 두 자리는 서로 부딪히는 축이에요. 변화·이동이 잦은 대신, 한번 정리되면 오히려 시원해지는 자리입니다.`,
        ...
        out.push({type:"육합", a,b,posA:ka,posB:kb, priority:6,
          easy:`${tag} 육합(合) — 이 두 자리는 서로 잘 묶이는 사이예요. 협력·인연이 붙기 쉬운 자리이자, 가끔 발이 묶이기도 합니다.`,
  ...
  IREL_TRIADS.forEach((tri,ti)=>{
    ...
    if (uniq.length===3){
      out.push({type:"삼합", elem:IREL_TRIAD_ELEM[ti], priority:8,
        easy:`${glyphs} 삼합(${IREL_TRIAD_ELEM[ti]}국) 완성 — 세 글자가 뭉쳐 ${ELEM_KO[IREL_TRIAD_ELEM[ti]]} 기운의 큰 세력을 이뤄요. 이 사주의 뚜렷한 힘의 방향입니다.`,
      ...
      out.push({type:"반합", elem:IREL_TRIAD_ELEM[ti], priority:5,
        easy:`${BRANCHES_KO[king]}${BRANCHES_KO[other]} 반합 — ${ELEM_KO[IREL_TRIAD_ELEM[ti]]} 기운이 절반쯤 뭉친 자리. 나머지 한 글자(${glyphs})가 운에서 오면 힘이 완성됩니다.`,
  ...
  IREL_SAMHYEONG.forEach(tri=>{
    ...
    out.push({type:"삼형", priority: full?8:4,
      easy:`${have.map(x=>BRANCHES_KO[x]).join("")} ${full?"삼형 완성":"형(刑) 기운"} — ${full?"강한 시비·수술·계약 변동의 축이라 그 해엔 서류를 꼼꼼히.":"은근한 마찰의 자리. 무리한 확장보다 정리에 강한 구조예요."}`,
  ...
  for(let i=0;i<keys.length;i++) for(let j=i+1;j<keys.length;j++){
    ...
    if (IREL_STEMHAP[sa]===sb){
      ...
      out.push({type:"천간합", priority: inv?8:5, involvesDay:inv,
        easy:`${tag} 합(合)${inv?" — 일간이 직접 합하는 자리라 특히 크게 작용해요":""}. 두 기운이 손잡아 ${ELEM_KO[IREL_STEMHAP_ELEM[sa]]}으로 향하는 배합입니다.`,
...
function godStructures(pillars){
  ...
  if (sikSang && jae && !weak){
    if (has(g,"식신")){
      push("식신생재", 9,
        `식신생재 — 꾸준히 파고든 전문성·솜씨(식신)가 안정적인 수입(재성)으로 이어지는 배치예요. 한 우물을 깊게 파 벌어먹는, 축재가 단단한 구조입니다.`,
        `식신생재 — 식신의 전일한 생재. 안정 축재형, 재원이 꾸준하고 도식만 관리하면 튼튼.`);
    } else {
      push("상관생재", 9,
        `상관생재 — 순발력·기획·화제성(상관)이 곧바로 돈(재성)이 되는 배치예요. 폭발력·확장성이 큰 대신 수입 진폭도 커서, 잘 벌 때 묶어두는 게 관건입니다.`,
        `상관생재 — 상관의 총명·확장형 생재. 진폭 크고 기회 민감, 패인·강제 축장이 안정화 장치.`);
    }
  }
  if (has(g,"상관") && has(g,"정관")){
    push("상관견관", 8,
      `상관견관 — 톡톡 튀는 표현(상관)과 규칙·상사(정관)가 한 사주에 같이 있어요. 실력은 인정받되, 말이 권위를 칠 때 탈이 나니 표현 수위 조절이 평생 무기입니다.`,
  ...
  if (weak && (grp["재성"]>=3 || (grp["재성"]>=2 && grp["관성"]>=2))){
    ...
    push("재다신약", 9,
      `재다신약 — 돈그릇(재성)은 큰데 그걸 감당할 내 힘(일간)이 여린 구조예요.${jaesal?" 게다가 그 돈이 다시 책임·압박(관살)으로 이어져(재생살) 부담이 겹칩니다." : ""} 고전 이름은 '부잣집의 가난한 사람'이지만, 처방도 분명합니다 — 내 편(비겁·인성)이 들어오는 운에 큰돈이 잡혀요.`,
      `재다신약${jaesal?"(재생살)":""} — 재중신경(財重身輕). 비겁·인성 부신운에 발재, 재→살 유통으로 부담 가중.`);
  }
  if (grp["관성"]>=3 && weak){
    push("관살태과", 7,
      `관살이 많은 구조 — 책임·압박(관성)이 큰데 몸이 여린 편이라, 다 떠안기보다 '나를 채워주는 자격·공부(인성)'로 통관시키는 게 이 사주의 살길이에요.`,
      `관살태과 신약 — 살중신경. 인성 통관(살인상생)이 정법, 식상 제살은 신약 시 부담.`);
  }
  if (has(g,"편관") && (has(g,"식신")||has(g,"상관")) && !weak){
    push("식상제살", 7,
      `식상제살 — 나를 밀어붙이는 압박(편관)을 재능(식상)으로 되받아치는 구조예요. 시련이 클수록 실력으로 눌러 이기는, 카리스마가 서는 배치입니다.`,
      `식상제살 — 제살태과 여부만 관리하면 권력 구조. 신주 강약이 제화 강도를 좌우.`);
  }
  if (has(g,"편관") && (has(g,"편인")||has(g,"정인"))){
    const looseSanggwan = has(g,"상관") && has(g,"정관");
    push("살인상생", 6,
      `살인상생 — 시련·압박(편관)이 공부·자격(인성)을 거쳐 내 실력으로 바뀌는 배치예요. 고생이 스펙이 되는, 늦게라도 크게 되는 구조입니다.${looseSanggwan?" 단 상관이 정관을 치는 부분이 있어, 그 표현 수위 조율이 이 흐름을 살리는 선행 과제입니다." : ""}`,
      `살인상생 — 살→인→일 유통, 귀격 배합의 정로.${looseSanggwan?" 상관견관 공존 — 거관 리스크 관리가 전제." : ""}`);
  }
  if (has(g,"정관") && (has(g,"편인")||has(g,"정인")) && !has(g,"상관")){
    push("관인상생", 6,
      `관인상생 — 직장·명예(정관)와 공부·자격(인성)이 서로를 살려주는 배치예요. 조직 안에서 자격을 무기로 올라가는 정통 성공 루트를 타고났습니다.`,
      `관인상생 — 관→인→신의 정순 유통, 청귀(淸貴)의 배합.`);
  }
  if (grp["비겁"]>=3 && jae){
    push("군겁쟁재", 6,
      `군겁쟁재 — 내 편도 많지만(비겁) 그만큼 돈(재성)을 나눠 갖는 구조라, 동업·돈거래·보증이 이 사주가 돈을 잃는 1순위 통로예요. 사람과 돈은 섞지 않는 게 철칙.`,
      `군겁쟁재 — 비겁이 재를 분탈. 재정 분리·단독 운용이 처방, 겁재운 가중 주의.`);
  }
  if ((has(g,"편인")||has(g,"정인")) && has(g,"식신") && grp["인성"]>=2 && grp["식상"]<=1){
    push("도식", 5,
      `인성이 식신을 누르는 편인도식 기미 — 생각·자격은 많은데 '실행·표현'이 눌리기 쉬워요. 배운 걸 반드시 밖으로 꺼내 결과물로 만드는 습관이 보약입니다.`,
      `편인도식(효신탈식) 기미 — 인다신왕 시 식상 설기 통로 확보가 관건.`);
  }
  if (has(g,"상관") && (has(g,"편인")||has(g,"정인"))){
    const jeongIn = has(g,"정인"), gyeongwan = has(g,"정관");
    push("상관패인", 5,
      `상관패인 — 튀는 재능(상관)을 ${jeongIn?"정통 공부·자격(정인)":"전문 기예·직관(편인)"}이 다듬어주는 배치예요. ${gyeongwan?"위 상관견관을 바로 이 인성이 풀어주는 구조라 — ":""}자격을 먼저 갖추고 표현할 때 이 재능이 칼집을 얻습니다.`,
      `상관패인 — ${jeongIn?"정인":"편인"}이 상관을 제어.${gyeongwan?" 상관견관의 해소 기제(패인)로 작동." : ""} 파격을 성격으로 전환하는 정법.`);
  }
  if (grp["인성"]===0){
    push("무인성", 4,
      `사주에 인성(공부·자격·기댈 언덕)이 드러나 있지 않아요 — 남의 도움보다 실전·경험으로 배우는 자수성가형이라는 뜻. 자격증 하나를 의식적으로 챙기면 약점이 메워집니다.`,
      `무인성 — 자화(自化)·실전 학습형. 인성운 도래 시 문서·자격 기회로 발현.`);
  }
  if (grp["재성"]===0){
    push("무재", 4,
      `사주에 재성(돈 글자)이 드러나 있지 않아요 — 돈이 없다는 게 아니라 돈이 1순위 동기가 아니라는 뜻. 실력·명예를 쌓으면 돈이 따라오는 유형이에요.`,
